...
async function main() {
  ...
  const sqlPath = path.join(process.cwd(), 'scratch', 'vocab-v2-migration.sql');
  const sql = fs.readFileSync(sqlPath, 'utf8');

  console.log('Attempting to execute SQL migration via Supabase RPC...');
  
  // Try common custom RPC names if they exist, or check response
