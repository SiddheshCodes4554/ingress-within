...
async function main() {
  ...
  const sqlPath = path.join(process.cwd(), 'scratch', 'report-data-integrity-migration.sql');
  const sql = fs.readFileSync(sqlPath, 'utf8');

  console.log('Attempting to execute report integrity SQL migration via Supabase RPC...');
