...
async function main() {
  ...
  console.log('Adding column vocab_backfill_completed to profiles...');
  const { data, error } = await db.rpc('exec_sql', { 
    sql_query: 'ALTER TABLE profiles ADD COLUMN IF NOT EXISTS vocab_backfill_completed BOOLEAN DEFAULT FALSE;' 
  });
