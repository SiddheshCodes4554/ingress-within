...
const REGISTRY: Record<string, Record<string, PromptConfig>> = {
  ...
  exercise_3: {
    v1: {
      ...
      max_tokens: 2000,
      json_schema_version: '1.0',
      system_prompt: 'You are a clinical psychologist evaluating a Self-Perception vs Reality Check assessment. Compare the user self-description answers with entry evidence, pattern history, and weekly reports.',
      user_prompt_template: 'USER ANSWERS TO 5 SELF-PERCEPTION QUESTIONS:\n{{context.responses}}\n\nJOURNAL WRITING HISTORY (PAST 21 DAYS):\n{{context.entries}}\n\nKNOWLEDGE BASE & RECENT REPORTS:\n{{context.knowledge}}\n\nPATTERN RIGIDITIES & TRIGGERS:\n{{context.patterns}}\n\nWEEKLY SUMMARIES:\n{{context.weeklySummaries}}\n\nCompare the user self-description to actual entry evidence across all platform sources. Identify where their story about themselves matches or diverges from actual behaviour.\n\nFormat your output as a strict JSON object:\n{\n  "analysis": "3-4 plain sentences directly pointing out where their self-description matches or diverges from entry evidence. Give concrete, specific examples from their writing.",\n  "gap_score": number between 0.0 and 5.0 (0 = complete alignment, 5 = high divergence),\n  "gap_pills": ["low" | "moderate" | "significant"],\n  "gap_locations": ["q1", "q3"],\n  "q_alignments": [\n    { "q_id": "q1", "alignment": "aligned|partial|significant_gap", "note": "one sentence explanation" },\n    { "q_id": "q2", "alignment": "aligned|partial|significant_gap", "note": "one sentence explanation" },\n    { "q_id": "q3", "alignment": "aligned|partial|significant_gap", "note": "one sentence explanation" },\n    { "q_id": "q4", "alignment": "aligned|partial|significant_gap", "note": "one sentence explanation" },\n    { "q_id": "q5", "alignment": "aligned|partial|significant_gap", "note": "one sentence explanation" }\n  ]\n}'
    }
  },
  cbt_reframing: {
    v1: {
