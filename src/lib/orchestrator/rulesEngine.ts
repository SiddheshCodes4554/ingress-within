...
export const ORCHESTRATION_RULES: OrchestrationRule[] = [
  ...
  // 10. AssessmentCompleted -> Queue Exercise Engine
  {
    name: 'AssessmentCompleted -> Queue Exercise Engine',
    triggerEvent: 'AssessmentCompleted',
    conditions: async () => true,
    action: async (userId, payload, ctx) => {
      const jobId = await ctx.enqueueJob(userId, 'exercise', `AssessmentCompleted:${payload.assessment_id}`);
      await ctx.queueRegistry.addJob('exercise_insight_generation', `exercise_${payload.assessment_id}`, {
        exercise_id: payload.assessment_id,
        user_id: userId,
        orchestrator_job_id: jobId
      });
    }
  ...
  // 11. JournalSubmitted -> Trigger Exercise Unlock Check
  {
    name: 'JournalSubmitted -> Trigger Exercise Unlock Check',
    triggerEvent: 'JournalSubmitted',
    conditions: async () => true,
    action: async (userId, payload, ctx) => {
      const { data: activeCycle } = await supabase
      ...
      if (activeCycle) {
        ...
        const currentDay = entry?.cycle_day || 1;

        const { ExerciseUnlockService } = await import('../exercises/exerciseUnlockService');
        await ExerciseUnlockService.processUnlocks(userId, activeCycle.id, timezone, currentDay);
      }
    }
  },

  // 12. AssessmentCompleted -> Trigger Exercise Unlock Check
  {
    name: 'AssessmentCompleted -> Trigger Exercise Unlock Check',
    triggerEvent: 'AssessmentCompleted',
    conditions: async () => true,
    action: async (userId, payload, ctx) => {
      const { data: activeCycle } = await supabase
  ...
  // 13. exercise.completed -> Queue Knowledge Processing
  {
    name: 'exercise.completed -> Queue Knowledge',
    triggerEvent: 'exercise.completed',
    conditions: async () => true,
    action: async (userId, payload, ctx) => {
      const jobId = await ctx.enqueueJob(userId, 'knowledge', `exercise.completed:${payload.instance_id}`);
      await ctx.queueRegistry.addJob('knowledge_processing', `knowledge_exercise_${payload.instance_id}`, {
        event_id: payload.instance_id,
        user_id: userId,
        cycle_id: payload.cycle_id,
        orchestrator_job_id: jobId
      });
  ...
  // 14. exercise.completed -> Queue Patterns Extraction
  {
    name: 'exercise.completed -> Queue Patterns',
    triggerEvent: 'exercise.completed',
    conditions: async () => true,
    action: async (userId, payload, ctx) => {
      const jobId = await ctx.enqueueJob(userId, 'patterns', `exercise.completed:${payload.instance_id}`);
      await ctx.queueRegistry.addJob('pattern_processing', `pattern_exercise_${payload.instance_id}`, {
        entry_id: payload.instance_id,
        user_id: userId,
        cycle_id: payload.cycle_id,
