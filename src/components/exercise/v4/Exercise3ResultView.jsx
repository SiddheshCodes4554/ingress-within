...
import { RotateCw, AlertCircle, X, CheckCircle2 } from 'lucide-react';
...
export default function Exercise3ResultView({ instanceId, onClose }) {
  ...
  if (error || !resultData) {
    return (
      <div className="fixed inset-0 z-50 bg-[#ECEFF0] text-[#1E2A2E] flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-xs space-y-4 bg-white p-6 rounded-2xl shadow-sm border border-line">
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#ECEFF0] text-[#1E2A2E] flex flex-col justify-between p-6 font-sans overflow-y-auto">
      <div className="max-w-[480px] w-full mx-auto py-6 space-y-6">
        {/* Top Bar Logo & Close */}
        <div className="flex items-center justify-between border-b border-[#1E2A2E]/10 pb-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1 rounded-full hover:bg-black/5 text-[#4A6A64] hover:text-[#1E2A2E] transition-colors cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="space-y-4">
          ...
          {/* Section Label */}
          <div className="text-[10px] font-semibold tracking-[0.12em] uppercase text-[#4A6A64]/55 pt-2">
