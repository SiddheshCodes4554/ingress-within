...
export default function Navbar() {
  ...
  const getLinkClass = (path) => {
    ...
    return `font-sans text-[13.5px] border-b-2 pb-[4px] transition-all no-underline ${
      isActive 
  ...
  const getMobileLinkClass = (path) => {
    const isActive = activePath === path;
    return `font-sans text-sm transition-all py-2 border-b border-primary/5 no-underline ${
      isActive ? 'text-primary font-medium pl-2 border-l-2 border-accent' : 'text-mid font-normal hover:text-primary'
    }`;
  };
    ...
    <nav className="fixed top-0 left-0 right-0 z-[100] px-[5%] h-[68px] flex items-center justify-between bg-mint-grey/95 backdrop-blur-[12px] border-b border-primary/8 transition-all duration-300">
      ...
      {mobileMenuOpen && (
        <div className="absolute top-[68px] left-0 right-0 bg-mint-grey border-b border-primary/8 shadow-md flex flex-col p-6 gap-4 md:hidden animate-[fadeIn_0.2s_ease-out]">
          ...
          <a href="/faq" onClick={() => setMobileMenuOpen(false)} className={getMobileLinkClass('/faq')}>FAQ</a>
